...
export type MigrationStatus = Readonly<{ name: string; checksum: string | null; storedChecksum: string | null; state: MigrationState }>;
const checksum = (contents: string): string => createHash('sha256').update(contents, 'utf8').digest('hex');
...
async function migrationFiles(): Promise<Map<string, Readonly<{ sql: string; checksum: string }>>> {
  const names = (await readdir(migrationsDirectory)).filter((name) => /^\d+.*\.sql$/u.test(name)).sort();
  return new Map(await Promise.all(names.map(async name => { const sql=await readFile(join(migrationsDirectory,name),'utf8'); return [name,{sql,checksum:checksum(sql)}] as const; })));
}
export async function migrationStatus(database: SqlClient): Promise<readonly MigrationStatus[]> {
  ...
  const applied=await database.query<{name:string;checksum:string|null}>('SELECT name,checksum FROM schema_migrations ORDER BY name');
  const rows:MigrationStatus[]=[];
  for(const row of applied.rows){const file=files.get(row.name);rows.push({name:row.name,checksum:file?.checksum??null,storedChecksum:row.checksum,state:!file||!row.checksum||row.checksum!==file.checksum?'drifted':'applied'});files.delete(row.name);}
  for(const [name,file] of files)rows.push({name,checksum:file.checksum,storedChecksum:null,state:'pending'});
...
export async function migrate(database: Database): Promise<void> {
  ...
  const legacy=status.filter(item=>item.state==='drifted'&&item.checksum!==null&&item.storedChecksum===null);
  if(legacy.length)await database.transaction(async tx=>{for(const item of legacy){const trusted=legacyChecksums[item.name];if(!trusted||trusted!==item.checksum)throw new Error(`Migration integrity check failed: ${item.name}`);await tx.query('UPDATE schema_migrations SET checksum=$2 WHERE name=$1 AND checksum IS NULL',[item.name,trusted]);}});
  status=await migrationStatus(database);
  const drifted=status.filter(item=>item.state==='drifted');
  if(drifted.length)throw new Error(`Migration integrity check failed: ${drifted.map(item=>item.name).join(', ')}`);
  const files=await migrationFiles();
  for (const item of status) {
    if (item.state === 'applied') continue;
    const file=files.get(item.name);if(!file)throw new Error(`Migration file missing: ${item.name}`);
    await database.transaction(async (tx) => { await tx.query(file.sql); await tx.query('INSERT INTO schema_migrations(name,checksum) VALUES ($1,$2)', [item.name,file.checksum]); });
  }
