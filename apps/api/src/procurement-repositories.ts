...
export class PostgresProcurementRepository {
  ...
  public async list(
    view: 'suppliers' | 'rfqs' | 'quotes' | 'orders' | 'receipts' | 'inventory',
    context: Context,
  ) {
    company(context);
    const queries = {
      ...
      inventory: `SELECT jsonb_build_object('itemVersionId',b.item_version_id,'lotId',b.lot_id,'locationId',b.location_id,'quantity',b.quantity,'lastMovementAt',b.last_movement_at,'sku',i.sku,'itemName',i.name,'lotNumber',lot.lot_number,'qualityStatus',lot.quality_status,'locationCode',loc.code,'movements',coalesce((SELECT jsonb_agg(to_jsonb(m) ORDER BY m.sequence) FROM inventory_movements m WHERE m.tenant_id=b.tenant_id AND m.lot_id=b.lot_id AND m.location_id=b.location_id),'[]'::jsonb)) item FROM inventory_balances b JOIN manufacturing_item_versions v ON v.id=b.item_version_id AND v.tenant_id=b.tenant_id JOIN manufacturing_items i ON i.id=v.item_id AND i.tenant_id=v.tenant_id JOIN inventory_lots lot ON lot.id=b.lot_id AND lot.tenant_id=b.tenant_id JOIN inventory_locations loc ON loc.id=b.location_id AND loc.tenant_id=b.tenant_id WHERE b.tenant_id=$1 ORDER BY i.sku,lot.lot_number,loc.code`,
    } as const;
    return (
