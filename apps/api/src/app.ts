...
import { DomainError, type EmployeeRepository, type OrganizationRepository } from '@kingturf/domain';
import type { EmployeeDto, ErrorCode, ErrorEnvelope, HealthStatus, OrganizationDto, PermissionKey } from '@kingturf/types';
import { authorizeQuery } from './policy.ts';
import type { AuthenticationService } from './security.ts';

type Json = HealthStatus | ErrorEnvelope | OrganizationDto | EmployeeDto | readonly OrganizationDto[] | readonly EmployeeDto[] | Readonly<Record<string, unknown>>;
...
export type ApiDependencies = Readonly<{ auth: AuthenticationService; organizations: OrganizationRepository; employees: EmployeeRepository }>;
  ...
  return { async dispatch(request) {
    ...
    try {
      ...
      if(organizationMatch){
        ...
        if(request.method==='POST'&&!id){authorizeQuery(context,'organization:create');const b=objectBody(request.body);const created=await dependencies.organizations.create({ownerOrganizationId:context.actor.companyId,parentId:typeof b.parentId==='string'?b.parentId:null,code:string(b.code,'code'),name:string(b.name,'name'),locale:typeof b.locale==='string'?b.locale:'zh-CN',currency:typeof b.currency==='string'?b.currency:'CNY',active:true},context.actor);return{statusCode:201,body:created};}
        if(request.method==='PATCH'&&id){const b=objectBody(request.body);const fields=Object.keys(b).filter(k=>k!=='version');authorizeQuery(context,'organization:update',fields);const updated=await dependencies.organizations.update(id,context.actor.companyId,b,version(b.version),context.actor);return{statusCode:200,body:updated};}
      }
      const employeeMatch=/^\/api\/v1\/employees(?:\/([0-9a-f-]+))?$/u.exec(request.pathname);
      if(employeeMatch){
        const id=employeeMatch[1];
        if(request.method==='GET'&&!id){const grant=authorizeQuery(context,'employee:read');return{statusCode:200,body:await dependencies.employees.list(context.actor.companyId,grant.scopes,context.actor.employeeId)};}
        if(request.method==='GET'&&id){const grant=authorizeQuery(context,'employee:read');const found=await dependencies.employees.findById(id,context.actor.companyId,grant.scopes,context.actor.employeeId);return found?{statusCode:200,body:found}:error(404,'not_found','Employee not found',correlationId);}
        if(request.method==='POST'&&!id){authorizeQuery(context,'employee:create');const b=objectBody(request.body);const created=await dependencies.employees.create({companyId:context.actor.companyId,organizationId:string(b.organizationId,'organizationId'),employeeNumber:string(b.employeeNumber,'employeeNumber'),displayName:string(b.displayName,'displayName'),email:string(b.email,'email'),active:true},context.actor);return{statusCode:201,body:created};}
        if(request.method==='PATCH'&&id){const b=objectBody(request.body);const fields=Object.keys(b).filter(k=>k!=='version');const grant=authorizeQuery(context,'employee:update',fields);const updated=await dependencies.employees.update(id,context.actor.companyId,b,version(b.version),context.actor,grant.scopes);return{statusCode:200,body:updated};}
      }
      if(/^\/api\/v1\/(roles|permissions|grants|assignments)/u.test(request.pathname)){const capability:PermissionKey=`authorization:${request.method==='GET'?'read':'manage'}`;authorizeQuery(context,capability);return error(404,'not_found','Authorization resource not found',correlationId);}
