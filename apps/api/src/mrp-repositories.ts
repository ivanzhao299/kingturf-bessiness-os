      ...
      const inputs = (
        await tx.query<{ snapshot: JsonObject }>(
          `SELECT jsonb_build_object(
            ...
            'inventory',coalesce((SELECT jsonb_agg(to_jsonb(b) ORDER BY b.item_version_id,b.lot_id,b.location_id) FROM inventory_balances b JOIN inventory_lots l ON l.id=b.lot_id AND l.tenant_id=b.tenant_id WHERE b.tenant_id=$1 AND l.quality_status='RELEASED'),'[]'::jsonb),
      ...
      const calculations = await tx.query<{ id: string }>(
        `WITH RECURSIVE exploded(item_version_id,required_at,quantity,path,depth) AS (
          ...
          SELECT g.*,coalesce(p.safety_stock,0) safety_stock,coalesce(p.minimum_order_quantity,0) minimum_order_quantity,
            ...
            coalesce((SELECT sum(b.quantity) FROM inventory_balances b JOIN inventory_lots lot ON lot.id=b.lot_id AND lot.tenant_id=b.tenant_id WHERE b.tenant_id=$1 AND b.item_version_id=g.item_version_id AND lot.quality_status='RELEASED'),0)::numeric(24,6) on_hand,
