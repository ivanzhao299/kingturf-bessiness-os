...
import { PostgresAuthorizationRepository, PostgresEmployeeRepository, PostgresOrganizationRepository, PostgresSecurityStore } from './repositories.ts';
...
const app=buildApp({auth,organizations:new PostgresOrganizationRepository(database),employees:new PostgresEmployeeRepository(database),authorization:new PostgresAuthorizationRepository(database)});
