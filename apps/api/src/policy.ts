...
export type AuthorizedQuery = Readonly<{
  context: AuthorizationContext;
  scopes: readonly DataScope[];
  anchors: readonly Readonly<{ scope: DataScope; organizationId: string | null }>[];
}>;
export function authorizeQuery(
  ...
  return {
    context: authorized,
    scopes: authorized.permissions.get(capability)?.scopes ?? [],
    anchors: authorized.scopeAnchors?.get(capability) ?? [],
  };
}

export function dataScopeSql(
  scopes: readonly DataScope[],
  employeeAlias = 'e',
  startParameter = 1,
  ...
  const add = (
    scope: DataScope,
    expression: (parameter: string) => string,
    value: string,
  ): void => {
    if (validScopes.includes(scope)) {
      clauses.push(expression(`$${String(startParameter + values.length)}`));
      values.push(value);
    }
  };
  ...
  add(
    'TEAM',
    (parameter) =>
      `EXISTS (SELECT 1 FROM organization_scope_relationships osr WHERE osr.ancestor_id = ${parameter} AND osr.descendant_id = ${employeeAlias}.organization_id AND osr.depth <= 1)`,
    'actorOrganizationId',
  );
  add(
    'DEPARTMENT',
    (parameter) =>
      `EXISTS (SELECT 1 FROM organization_scope_relationships osr WHERE osr.ancestor_id = ${parameter} AND osr.descendant_id = ${employeeAlias}.organization_id)`,
    'actorDepartmentId',
  );
  add(
    'REGION',
