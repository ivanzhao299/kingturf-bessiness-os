...
export type AuthorizedQuery = Readonly<{ context: AuthorizationContext; scopes: readonly DataScope[] }>;
export function authorizeQuery(context: AuthorizationContext | null, capability: PermissionKey, fields: readonly string[] = []): AuthorizedQuery {
  ...
  return { context: authorized, scopes: authorized.permissions.get(capability)?.scopes ?? [] };
}
