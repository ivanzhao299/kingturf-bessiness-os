...
function dependencies(authContext:AuthorizationContext|null){
  ...
  const assign=vi.fn(()=>Promise.resolve());const grantPermission=vi.fn(()=>Promise.resolve());const grantScope=vi.fn(()=>Promise.resolve({id:targetId,employeeId,permissionId:organizationId,scope:'TEAM' as const,organizationId}));
  const authorization={listRoles:vi.fn(()=>Promise.resolve([])),createRole:vi.fn(()=>Promise.resolve({id:targetId,organizationId:companyId,code:'ADMIN',name:'Admin',version:1})),listPermissions:vi.fn(()=>Promise.resolve([])),createPermission:vi.fn(),listGrants:vi.fn(()=>Promise.resolve([])),grant:grantPermission,revoke:vi.fn(()=>Promise.resolve()),listAssignments:vi.fn(()=>Promise.resolve([])),assign,unassign:vi.fn(()=>Promise.resolve()),listScopeGrants:vi.fn(()=>Promise.resolve([])),grantScope,revokeScope:vi.fn(()=>Promise.resolve())} as unknown as AuthorizationRepository;
  return {auth,organizations,employees,authorization,assign,grantPermission,grantScope,logout};
}
...
describe('authorization management API',()=>{
  ...
  it('validates and secures direct typed scope grants',async()=>{const deps=dependencies(context(grant('authorization:manage')));expect((await dispatch(deps,'POST','/api/v1/scope-grants',{employeeId,permissionId:organizationId,scope:'TEAM',organizationId})).statusCode).toBe(201);expect(deps.grantScope).toHaveBeenCalledWith({employeeId,permissionId:organizationId,scope:'TEAM',organizationId},expect.objectContaining({companyId}),expect.any(String));expect((await dispatch(deps,'POST','/api/v1/scope-grants',{employeeId,permissionId:organizationId,scope:'TEAM'})).statusCode).toBe(400);});
  it('rejects malformed UUIDs and unexpected administration fields',async()=>{const deps=dependencies(context(grant('authorization:manage')));expect((await dispatch(deps,'POST','/api/v1/assignments',{employeeId:'not-a-uuid',roleId:organizationId})).statusCode).toBe(400);expect((await dispatch(deps,'POST','/api/v1/roles',{code:'ADMIN',name:'Admin',organizationId:companyId})).statusCode).toBe(400);});
});
