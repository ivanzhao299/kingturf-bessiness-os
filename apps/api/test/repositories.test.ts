...
describe('PostgreSQL authorization repositories', () => {
  it('rejects cross-company organization updates before opening a transaction', async () => {
    ...
    await expect(
      repository.update(
        ...
        { name: 'Adversarial' },
        1,
        { employeeId: 'actor', companyId: 'company-a' },
  ...
  it('includes DataScope and destination organization ownership in employee updates', async () => {
    ...
    const transaction = vi.fn(async (work: (tx: SqlClient) => Promise<unknown>) =>
      work({ query } as SqlClient),
    );
    ...
    await expect(
      repository.update(
        ...
        1,
        { employeeId: 'actor', companyId: 'company' },
        ['SELF'],
        [],
        '00000000-0000-4000-8000-000000000001',
      ),
  ...
  it('deterministically unions scopes and field allowlists from multiple roles', async () => {
            ...
            rows: [
              {
                ...
                field_allowlist: ['displayName'],
                data_scopes: ['SELF'],
              },
              {
                capability: 'employee:update',
                field_allowlist: ['email'],
                data_scopes: ['COMPANY', 'SELF'],
              },
            ],
            rowCount: 2,
          })
    ...
    expect(result?.permissions.get('employee:update')).toEqual({
  ...
  it('treats any unrestricted field grant as unrestricted when roles are merged', async () => {
    ...
    const store = new PostgresSecurityStore({ transaction } as unknown as Database);
    expect(
      (await store.resolveSession('hash', new Date()))?.permissions.get('employee:read'),
    ).toEqual({ fields: null, scopes: ['SELF', 'TEAM'] });
  });
    ...
    const transaction = vi.fn(async (work: (tx: SqlClient) => Promise<unknown>) =>
      work({
        query: vi
          ...
          .mockResolvedValueOnce({
            rows: [{ capability: 'employee:read', field_allowlist: null, data_scopes: ['SELF'] }],
            rowCount: 1,
          })
          .mockResolvedValueOnce({
            rows: [{ capability: 'employee:read', scope: 'TEAM', scope_organization_id: 'team' }],
            rowCount: 1,
          }),
      }),
    );
    const result = await new PostgresSecurityStore({
      transaction,
    } as unknown as Database).resolveSession('hash', new Date());
    expect(result?.permissions.get('employee:read')?.scopes).toEqual(['SELF']);
    expect(result?.scopeAnchors?.get('employee:read')).toEqual([
      { scope: 'TEAM', organizationId: 'team' },
    ]);
  });
});
